...
- On templates, the list and the template name are Headline (24px rising to 30px at `sm`), the name editable in place. A group header is Title (17px/600) in the user's own casing, never uppercase, with a 13px muted tabular count on the same baseline. A template row's name is 17px/600 over a 13px muted tabular meta line.
...
- On exercises, the catalog title and an exercise's name are Headline (24px rising to 30px at `sm`). A muscle-group header on the catalog page is Title (17px/600) in sentence case with a 13px muted tabular count on its baseline, not the uppercase Column Label that heads groups inside the add-exercise sheet. A catalog row's name is 15px/500, and its value is 15px/600 tabular with a 13px muted "kg" over a 12px muted tabular caption.
...
### Templates
Both templates pages sit in the same 1024px column (`max-w-5xl`) as plans. The list header puts the Headline with a 14px muted tabular template count under it on the left, and the single blue "Nowy szablon" (44px, plus icon) on the right, full width on phones. Groups follow 24px below, 32px apart: "Pojedyncze treningi" first, then one group per plan. A group header is a 40px row holding the Title (17px/600, the plan's name in the user's casing) with a 13px muted tabular count on its baseline (stacked under the name on phones, so a long plan name is never cut), and on the right a quiet 40px Signal Blue "Otwórz plan" link pulled out to the edge like the dashboard's section links. Under each header, 8px down, the templates are rows in the Plan List Row grammar: the name at 17px/600, a 13px muted tabular line ("N ćwiczeń · muscle groups", or "Bez ćwiczeń"), the notes as a 13px muted truncated line when they exist, and a muted chevron. A row whose template has today's planner entry in progress opens its meta line with "W trakcie" in foreground weight 500, the same lead as the Workout List Row. An empty "Pojedyncze treningi" group is one Surface panel with a muted line and a quiet blue "Nowy szablon". With no templates at all, the page is one Surface empty panel (20px padding, max 576px) with a Title, a muted line, the blue "Nowy szablon", and a quiet blue link to plans.

The template detail opens with a 40px muted back link: to the owning plan (named) for a plan's day, to the list otherwise. The header has the name as Inline Text inside the Headline h1, then for a plan-owned template a 13px muted "Dzień planu" line behind a 14px clipboard icon with the plan's name as a foreground weight 500 link, then the notes as Inline Text. On the right sit the blue "Rozpocznij trening" (44px, 14px/600, a filled play icon), reading "Kontynuuj trening" when today's entry for the template is in progress, and a 44px Outline ⋯ square whose menu holds "Zaplanuj…" and "Usuń szablon" in Danger Red. On phones the header stacks and the blue action stretches beside the ⋯. While the template has no exercises, the start drops to a disabled Outline button with muted text and the page's blue moves to the editor's "Dodaj ćwiczenie". The body, 24px down, is one Template Editor in its page variant. "Zaplanuj…" opens a Sheet with one date field (defaulting to today), a full-width blue "Dodaj do planera", and a quiet blue "Otwórz planer" link under it.
...
### Exercises
Both exercise pages sit in the same 1024px column (`max-w-5xl`) as plans. The catalog header is the Headline with a 14px muted tabular line under it: the exercise count, then "· N z twoimi wynikami" when the user has records. 24px down, a stack 12px apart holds the search field and, on its own row below it, the muscle-group Segmented Control ("Wszystkie" plus the groups present, state in `?group=`), because eleven groups do not fit beside the search. The search is the Graphite Muted 44px field, full width on phones and at most 384px from `sm` (see Inputs / Fields). Changing the group starts with an empty search. Groups follow 32px down and 32px apart, in the muscle-group enum order, each a Title header with its count 12px above one Surface list panel of Catalog Rows. A search with no match is one Surface panel with a muted line naming the query and a 40px quiet blue "Wyczyść wyszukiwanie". If records fail to load, the catalog still renders without values, under a Danger Red Tint panel (12px corners, 14px text) that says so.

The exercise card opens with a 40px muted back link to the catalog, then 8px down the Headline name (balanced across lines) over the muscle group as a 14px muted quiet link to the filtered catalog (hovering to foreground, underlined). "Twoje rekordy" follows 32px down as a stats section: Title on the left and, only when records exist, the section-header link "Pełne statystyki" on the right (36px quiet Signal Blue with a trailing chevron, pulled out to the edge like the dashboard's section links), then the shared Record Strip. Without records the section is one Surface panel (12px corners, 16px padding, 20px sides from `sm`) with a 14px muted line. "Podobne ćwiczenia" follows 40px down with a 13px muted hint under its Title and a Surface list panel of Similar Rows. When there are no similar exercises, or they are not available yet for this exercise, the section is the same muted panel. A failed load in either section swaps in the Section Error for that section only. Neither exercise page carries a Signal Blue button: blue there is only quiet links and focus.
...
### Segmented Control
One look for every either/or switch: the planner's Tydzień/Miesiąc control, the stats range (30 dni / 3 mies. / rok / cały czas) and metric (1RM / top set / volume) switches, and the exercise catalog's muscle-group filter. A Graphite Muted track with 8px corners and 4px padding holds 36px options with 6px corners and 14px/500 text. The current option is a Surface fill with foreground text. The others are muted and hover to foreground. On stats and the exercise catalog, each option is a link, the state lives in the URL (`?range=`, `?metric=`, `?group=`), and the current one carries `aria-current`. The track scrolls sideways rather than wrapping when space runs out. The one exception is a choice inside a form (the AI plan goal), where the options are radio inputs laid out as a 2×2 grid of 40px options on the same track.
...
One training day as a compact target table, edited where it is read. It is shared by the plan detail (day variant) and the template page (page variant).
...
- **Table:** from `sm`, a Column Label header row between hairlines, then hairline-divided exercise rows on the Template Grid (see Layout). The exercise cell is the name at 15px/500 (truncating only from `sm`) over a 12px muted muscle group. The ↑↓ are 36px muted square icon buttons (8px corners, 30% opacity when disabled at the ends), and the ⋯ is a 40px round muted button whose menu holds "Zamień na podobne" (opening the Swap Sheet) above "Usuń z dnia" in Danger Red.
...
- **Page variant (template page):** the page header owns the name and the delete, so the section header drops the Inline Text name and the ⋯. It holds the exercise count as the heading at Title size (17px/600, tabular) with the save status on the same baseline at the right edge (13px muted, a polite live region, without the leading middle dot). An empty template shows a fuller 14px muted line, the footer loses its hairline, and "Dodaj ćwiczenie" becomes the page's solid Signal Blue button (44px, 16px padding, 14px/600), because adding an exercise is the only useful action there.

### Swap Sheet (plans)
"Zamień na podobne" from the Template Editor's exercise ⋯, built on the Sheet. The title names the exercise ("Zamień „…”"), and a 14px muted hint says the sets, reps, weight, and rest stay the same. Similar exercises load when it opens (a muted status line while loading). Candidates are 48px rows pulled out 8px on each side, with 8px corners and a Graphite Muted hover: the name at 15px on the left and the similarity as a plain 13px muted tabular percentage on the right. A candidate already in the day is disabled, its name muted and its percentage replaced by "Już w tym dniu". "Wybierz z całego katalogu", a 44px quiet blue text button, is always offered: under the list above a hairline, or as the only action under a muted line when there are no similar exercises or they are not available yet. It opens the logger's catalog sheet. A failed load shows a 14px Danger Red line and a quiet blue retry. A pick swaps the exercise in place, keeping its targets and position, and raises the neutral toast "Zamieniono „…” na „…”" with "Cofnij", which swaps it back. Picking an exercise already in the day from the catalog changes nothing and raises the error toast "„…” jest już w tym dniu.".
...
### Confirm Sheet
The confirmation for an irreversible action, built on the Sheet. The title names the thing ("Usunąć plan „…”?"), and the body is 14px muted copy that states the consequence honestly (what is deleted and what stays, such as scheduled entries keeping their dates). The footer holds an Outline cancel and the Danger confirm, both 48px: stacked with the confirm on top on phones, and right-aligned side by side from `sm`. While the action runs, the confirm is disabled at 60%. Deleting a plan's day from its template page says so in all three places: the title names the day and the plan ("Usunąć dzień „…” z planu „…”?"), the body says the day and its exercises leave that plan while scheduled entries stay in the planner without a template and logged workouts stay in history, and the confirm reads "Usuń dzień". A standalone template's sheet names the template and confirms with "Usuń szablon".
...
### Catalog Row (exercises)
Rows in one Surface list panel per muscle group (12px corners, hairline border, hairline dividers). Each row links to its exercise card, at least 56px tall with 10px vertical and 16px side padding (20px from `sm`) and a 16px gap, hovering to Graphite Muted. The name is 15px/500 and wraps. On the right, right-aligned, is the user's best estimated 1RM at 15px/600 tabular with a 13px muted "kg", over a 12px muted tabular "e1RM · date". A zero e1RM (bodyweight work) reads a 13px muted "Masa ciała". An exercise never done reads a 13px muted "Jeszcze nie robione" from `sm` and a muted "—" below `sm` (the full text stays for screen readers), so names hold one line on phones. When records failed to load the right side is left out. A 20px muted chevron closes the row.
- **Similar Row:** the Catalog Row on the exercise card, with the name over a 13px muted tabular "N% podobieństwa" and the same e1RM value and caption on the right, or "Jeszcze nie robione".
...
### Inputs / Fields
- **Style:** the search field is a Graphite Muted fill, 44px, 8px corners, and a leading muted search icon. On the exercise catalog it holds 15px text, takes a 2px Signal Blue ring on the whole field while focused, and, once there is a query, a themed 32px clear X at its right edge (6px corners, muted, hovering to a Surface fill and foreground). The WebKit native cancel button is hidden so only the themed clear shows. Auth fields are a Surface fill with a hairline border and 6px corners. Text fields in sheets (the plan name, the AI constraints) are a Graphite Muted fill with a hairline border and 8px corners, 44px tall for a single line, with 15px text.
...
- **Do** keep one Signal Blue button per plans and templates page: "Nowy plan" on the plans list, "Zaplanuj w planerze" on a plan (or "Dodaj dzień treningowy" in an empty plan's card), "Nowy szablon" on the templates list, and "Rozpocznij trening"/"Kontynuuj trening" on a template (or "Dodaj ćwiczenie" in an empty template, where the start drops to a disabled Outline). The other actions are Outline buttons, quiet blue text, or muted icon buttons.
- **Do** keep the exercise catalog and the exercise card free of a Signal Blue button. Blue there is only quiet links ("Pełne statystyki", "Wybierz z całego katalogu") and focus.
- **Do** swap an exercise in place from the Template Editor's ⋯, keeping its targets and position, show similarity as a plain percentage, and offer "Cofnij" in the toast.
